...
EfficiencyHists::EfficiencyHists(Context & ctx, const string & dirname, bool is_qcd, TopJetId id , Event::Handle<std::vector<pair<TopJet, TopJet> > > &topjetcollection):
Hists(ctx, dirname),
b_is_qcd(is_qcd),
m_id(id),
h_matched_pairs(topjetcollection)
{
  double pt_intervall[] = {0, 200, 400, 500, 600, 700, 800, 1000, 1200, 1400, 1600, 1800, 2000};

  pt_top = book<TH1F>("pt_gentop", "p_{T, topjet}", 12, pt_intervall);
  pt_matched = book<TH1F>("pt_top_matched", "p_{T, topjet}", 12, pt_intervall);
  pt_mismatched = book<TH1F>("pt_top_mismatched", "p_{T, topjet}", 12, pt_intervall);
  pt_gen = book<TH1F>("pt_gen", "p_{T, gen}", 12, pt_intervall);
