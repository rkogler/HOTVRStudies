...
// initialize event handle
  //  ...containing fastjet pseudojets
  Event::Handle<vector<fastjet::PseudoJet>> h_HOTVRjets;
  Event::Handle<vector<fastjet::PseudoJet>> h_VR_jets;
  Event::Handle<vector<fastjet::PseudoJet>> h_VR_jets_SD;
  Event::Handle<vector<fastjet::PseudoJet>> h_VR_jets_ISD;
  Event::Handle<vector<fastjet::PseudoJet>> h_parton_jets;
  // ...containing UHH2 Topjets
  Event::Handle<vector<TopJet>> h_top_hotvr_jets;
  Event::Handle<vector<TopJet>> h_top_parton_jets;
  Event::Handle<vector<TopJet>> h_HOTVRjets_tagged;
...
//  Event::Handle<vector<pair<TopJet, TopJet>>> h_matched_pairs_notag; //contains pair of not toptagged matched jets
...
//  hist_HOTVRjets.reset(new HOTVRStudiesHists(ctx, "HOTVRStudiesHists", is_qcd));
//  hist_NSubjettiness.reset(new NSubjettinessHists(ctx, "NSubjettinessHists"));
...
//for ROC curves
  ...
  TopJetId id_hotvr_base = HOTVRTopTag(0.8, 140., 220., 50); //definition of toptag
  //  TopJetId id_softdrop_base =  Type2TopTag(105., 220., Type2TopTag::MassType::groomed);
//NEW id_hotvr_base -> id_topjet
  ...
  for (int i = 0; i < n_points; ++i) //loop over working points and create hists
  {
    ...
    // sel_softdrop.push_back(std::move(sel));PseudoJet jet : matched_jets
    //
...
//CLUSTERING -> clusters hotvr jets or vr jets plus grooming
  clustering = new Clustering(m_clustering);
  clustering->cluster_jets(pseudojets); // cluster the pseudojets, you can define in Clustering.cxx which clustering to use
...
// files for Finley
  // implement some matching of these vr jets
  std::vector<fastjet::PseudoJet> matched_vr_jets;
  matching->run_matching(vr_jets, vr_jets_constituents, parton_jets);
  ...
  for (size_t j = 0; j < matched_vr_jets_and_constituents.size(); j++) {
...
//fill hists with VR jets -> for finleys study
...
// apply Top Tag
    if(toptagger->Is_tagged("sd", matched_jets[j])){
      ...
      if(parton_jet.pt()>200 && parton_jet.pt()<400)  hist_tagged_jets_200->fill_topjet(event, matched_jet);
      if(parton_jet.pt()>400 && parton_jet.pt()<600)  hist_tagged_jets_400->fill_topjet(event, matched_jet);
      if(parton_jet.pt()>600 && parton_jet.pt()<800)  hist_tagged_jets_600->fill_topjet(event, matched_jet);
      if(parton_jet.pt()>800 && parton_jet.pt()<1000)  hist_tagged_jets_800->fill_topjet(event, matched_jet);
      if(parton_jet.pt()>1000 && parton_jet.pt()<1200)  hist_tagged_jets_1000->fill_topjet(event, matched_jet);
    } // end if top tag
  } // end loop over matched jets

//set the event handle
  //these jets are still pseudojets (the clustered HOTVR jets)
  event.set(h_HOTVRjets, hotvr_jets);
  event.set(h_VR_jets, vr_jets);
  event.set(h_VR_jets_SD, vr_jets_SD);
  event.set(h_VR_jets_ISD, vr_jets_ISD);
  //these jets are still pseudojets (the clustered AK10 jets)
  event.set(h_parton_jets, parton_jets);
  //these jets are TopJets (the clustered and converted jets)
  event.set(h_top_hotvr_jets, _top_hotvr_jets);
  event.set(h_top_parton_jets, _top_parton_jets);
  ...
  //top tagged hotvr topjets
  event.set(h_HOTVRjets_tagged, matched_jets_tagged);
