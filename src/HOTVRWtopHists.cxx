...
hist_pt_W = book<TH1F>("p_{T}_W", "W p_{T} [GeV]", 200, 0, 2000);
hist_pt_b = book<TH1F>("p_{T}_b", "b p_{T} [GeV]", 200, 0, 2000);
hist_pt_top = book<TH1F>("p_{T}_top", "top p_{T} [GeV]", 200, 0, 2000);
...
hist_distance = book<TH1F>("distance", "distance [GeV]", 100, 0, 2); // HOTVR, b
hist_distance_top_b = book<TH1F>("distance_top_b", "distance (top, b) [GeV]", 100, 0, 2); // top, b
hist_distance_top_W = book<TH1F>("distance_top_W", "distance (top, W) [GeV]", 100, 0, 2); // top, W
hist_distance_W_b = book<TH1F>("distance_W_b", "distance (W, b) [GeV]", 100, 0, 2); // W, b
...
hist_pt_ratio = book<TH1F>("pt_ratio", "p_{T} (HOTVR jet)/ p_{T} (W jet)", 100, 0, 2);
hist_pt_ratio_HOTVR_top = book<TH1F>("pt_ratio_HOTVR_top", "p_{T} (HOTVR jet)/ p_{T} (top jet)", 100, 0, 2);
hist_pt_sum_W_b = book<TH1F>("p_{T}_sum_W_b", "p_{T} (W+b) [GeV]", 200, 0, 2000);
}

void HOTVRWtopHists::fill(const Event & event){}

void HOTVRWtopHists::fill_topjet(const Event & event, TopJet & jet, TopJet & parton_jet_W, TopJet & parton_jet_b, TopJet & parton_jet_top){
  if (b_is_qcd) {processed_events_qcd->Fill(0);}
    ...
    hist_pt_W->Fill(parton_jet_W.pt());
    hist_pt_b->Fill(parton_jet_b.pt());
    hist_pt_top->Fill(parton_jet_top.pt());
    ...
    hist_distance->Fill(deltaR(parton_jet_b, jet));
    hist_distance_top_b->Fill(deltaR(parton_jet_b, parton_jet_top));
    hist_distance_top_W->Fill(deltaR(parton_jet_W, parton_jet_top));
    hist_distance_W_b->Fill(deltaR(parton_jet_b, parton_jet_W));

    hist_pt_ratio->Fill(jet.pt()/parton_jet_W.pt());
    hist_pt_ratio_HOTVR_top->Fill(jet.pt()/parton_jet_top.pt());
    hist_pt_sum_W_b->Fill(parton_jet_W.pt()+parton_jet_b.pt());
