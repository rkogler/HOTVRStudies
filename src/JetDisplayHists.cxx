...
//jet display
  int bins=50;
 for(int o=0;o<20;o++) { // loop over 10 possible jets
...
// h_jets=ctx.get_handle<std::vector<std::vector<PseudoJet>>>("jets");
// h_rejected_cluster=ctx.get_handle<std::vector<std::vector<PseudoJet>>>("rejected_cluster");
// h_soft_cluster=ctx.get_handle<std::vector<std::vector<PseudoJet>>>("soft_cluster");
// h_rejected_subjets=ctx.get_handle<std::vector<std::vector<PseudoJet>>>("rejected_subjets");
// h_jet0_subjets=ctx.get_handle<std::vector<std::vector<PseudoJet>>>("jet0_subjets");
// h_jet1_subjets=ctx.get_handle<std::vector<std::vector<PseudoJet>>>("jet1_subjets");
// h_jet2_subjets=ctx.get_handle<std::vector<std::vector<PseudoJet>>>("jet2_subjets");
// h_jet3_subjets=ctx.get_handle<std::vector<std::vector<PseudoJet>>>("jet3_subjets");
}
...
//---------get all objects from event handle----------
  ...
  // const auto & jet0_subjets = event.get(h_jet0_subjets);
  // const auto & jet1_subjets = event.get(h_jet1_subjets);
  // const auto & jet2_subjets = event.get(h_jet2_subjets);
  // const auto & jet3_subjets = event.get(h_jet3_subjets);
...
if (SortedSubJets.size()>1) { // select only jets with more than one subjet
     ...
     std::cout << "subjet size" << SortedSubJets.size() << '\n';
	    TString hname3 = TString::Format("JetDisplay_jet%i_subjetmass",o);
   ...
   for(uint j=0; j< Had_Tops.size();j++) top->Fill(Had_Tops[j].phi(),Had_Tops[j].eta(),Had_Tops[j].pt());
   for(uint o=0;o<decay_products.size();o++) decay->Fill(decay_products[o].phi(),decay_products[o].eta(),decay_products[o].pt());
