...
class ModuleBASE: public uhh2::AnalysisModule {
 ...
 protected:
typedef std::unordered_map<std::string, std::unique_ptr<HOTVRJetsHists> >::iterator hfolder_itr;
std::unordered_map<std::string, std::unique_ptr<HOTVRJetsHists> > hfolder_;

  template<typename F> void book_HFolder(const std::string&, F*);
  template<typename F=HOTVRJetsHists> F* HFolder(const std::string&);
};
