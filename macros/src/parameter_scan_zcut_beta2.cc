...
ptbins[5]=", 1000 < p_{T} [GeV] < 1200";
  ...
  TH1F* hist_ttbar_SD = (TH1F*)files_ttbar.at(4)->Get("HOTVRJetsHists_"+jets[i]+ptmin[j]+ "/"+variables[k]);
  TH1F* hist_QCD_SD = (TH1F*)files_QCD.at(10)->Get("HOTVRJetsHists_"+jets[i]+ptmin[j]+"/"+variables[k]);

  TH1F* hist_ttbar_SD1 = (TH1F*)files_ttbar.at(9)->Get("HOTVRJetsHists_"+jets[i]+ptmin[j]+ "/"+variables[k]);
  TH1F* hist_QCD_SD1 = (TH1F*)files_QCD.at(3)->Get("HOTVRJetsHists_"+jets[i]+ptmin[j]+"/"+variables[k]);

  TH1F* hist_ttbar_SD2 = (TH1F*)files_ttbar.at(3)->Get("HOTVRJetsHists_"+jets[i]+ptmin[j]+ "/"+variables[k]);
  TH1F* hist_QCD_SD2 = (TH1F*)files_QCD.at(14)->Get("HOTVRJetsHists_"+jets[i]+ptmin[j]+"/"+variables[k]);

  TH1F* hist_ttbar_SD3 = (TH1F*)files_ttbar.at(13)->Get("HOTVRJetsHists_"+jets[i]+ptmin[j]+ "/"+variables[k]);
  TH1F* hist_QCD_SD3 = (TH1F*)files_QCD.at(2)->Get("HOTVRJetsHists_"+jets[i]+ptmin[j]+"/"+variables[k]);
  ...
  legend->AddEntry(hist_ttbar_SD,"HOTVR SD, #beta=2", "");
  ...
  legend->Draw();
  canvas->SaveAs("/nfs/dust/cms/user/albrecha/uhh2_102X_v2/HOTVRStudiesOutput/plots/SD_comparison/parameter_scan/ptt30_zcutscan/" + jets[i] + ptmin[j] +"_"+ variables[k]+"_beta2_ptt30.eps");
  }//end variables loop
  }//end pt loop
  }// end jets loop
