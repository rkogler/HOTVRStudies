...
int main(int argc){
  ...
  TFile *input1 = new TFile(" /nfs/dust/cms/user/albrecha/uhh2_102X_v2/HOTVRStudiesOutput/ttbar/VR_SD/uhh2.AnalysisModuleRunner.MC.ttbar_pythia8_flat_nnpdf23_VR_SD_24_02_z01_b01.root", "READ");
  TFile *input2 = new TFile(" /nfs/dust/cms/user/albrecha/uhh2_102X_v2/HOTVRStudiesOutput/ttbar/VR_SD/uhh2.AnalysisModuleRunner.MC.ttbar_pythia8_flat_nnpdf23_VR_SD_27_02_z001_b01.root", "READ");
  ...
  //beta=1
    ...
    //TF1* fit = new TF1("fit", "gaus", 75, 90); //W boson
    TF1* fit = new TF1("fit", "gaus", 160, 190); //top

    hotvr->Fit("fit", "R");
    double gaus_mean_hotvr=fit->GetParameter(1);
    mean_hotvr->SetBinContent(i+1, gaus_mean_hotvr);
    mean_hotvr->SetBinError(i+1, fit->GetParameter(2));

    vr->Fit("fit", "R");
    mean_vr->SetBinContent(i+1, (fit->GetParameter(1)));
    //mean_vr->SetBinError(i+1, (fit->GetParameter(1)));

    b1z01->Fit("fit", "R");
    mean_b1z01->SetBinContent(i+1, (fit->GetParameter(1)));
  //  mean_b1z01->SetBinError(i+1, (fit->GetParameter(1)));

    b1z001->Fit("fit", "R");
    mean_b1z001->SetBinContent(i+1, (fit->GetParameter(1)));
  //  mean_b1z001->SetBinError(i+1, (fit->GetParameter(1)));
  }
    ...
    canvas->SaveAs("/nfs/dust/cms/user/albrecha/uhh2_102X_v2/HOTVRStudiesOutput/plots/VR_SD/ttbar_flat_VR_SD_gaussian_top.eps");
