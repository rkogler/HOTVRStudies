...
ptbins[5]=", 1000 < p_{T} [GeV] < 1200";
  ...
  for (size_t i = 0; i < njets; i++) { // loop over jets
    for (size_t j = 0; j < nptmin; j++) { // loop over pt bins
      for (size_t k = 0; k < nvariables; k++) { // loop over variables
        ...
        TH1F* hist_ttbar_SD = (TH1F*)files_ttbar.at(10)->Get("HOTVRJetsHists_"+jets[i]+ptmin[j]+ "/"+variables[k]);
        TH1F* hist_QCD_SD = (TH1F*)files_QCD.at(1)->Get("HOTVRJetsHists_"+jets[i]+ptmin[j]+"/"+variables[k]);

        TH1F* hist_ttbar_SD1 = (TH1F*)files_ttbar.at(12)->Get("HOTVRJetsHists_"+jets[i]+ptmin[j]+ "/"+variables[k]);
        TH1F* hist_QCD_SD1 = (TH1F*)files_QCD.at(12)->Get("HOTVRJetsHists_"+jets[i]+ptmin[j]+"/"+variables[k]);

        TH1F* hist_ttbar_SD2 = (TH1F*)files_ttbar.at(0)->Get("HOTVRJetsHists_"+jets[i]+ptmin[j]+ "/"+variables[k]);
        TH1F* hist_QCD_SD2 = (TH1F*)files_QCD.at(15)->Get("HOTVRJetsHists_"+jets[i]+ptmin[j]+"/"+variables[k]);

        TH1F* hist_ttbar_SD3 = (TH1F*)files_ttbar.at(8)->Get("HOTVRJetsHists_"+jets[i]+ptmin[j]+ "/"+variables[k]);
        TH1F* hist_QCD_SD3 = (TH1F*)files_QCD.at(11)->Get("HOTVRJetsHists_"+jets[i]+ptmin[j]+"/"+variables[k]);
  ...
  legend->AddEntry((TObject*)0, "", "");
  legend->AddEntry(hist_ttbar_SD,"HOTVR SD, #beta=0", "");
  ...
  legend->Draw();
  canvas->SaveAs("/nfs/dust/cms/user/albrecha/uhh2_102X_v2/HOTVRStudiesOutput/plots/SD_comparison/parameter_scan/ptt30_zcutscan/" + jets[i] + ptmin[j] +"_"+ variables[k]+"_beta0_ptt30.eps");
  }//end variables loop
  }//end pt loop
  }// end jets loop
