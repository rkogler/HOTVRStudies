	...
	if ( file.is_open() ) {
	  while ( file ) {
		...
		if (strncmp(&(line.front()),"#",1)==0){
			...
			leave ol;
			sscanf( line.data(), "%d %d %d  %d", &ol.ind, &ol.parent1, &ol.parent2, &ol.child );
			//std::cout << "ind = " << ol.ind << ", p1 = " << ol.parent1 << ", p2 = " << ol.parent2 << ", child = " << ol.child << std::endl;
			if (tree.size()>1){
...
void ClusterTwoClusters(leave tl, std::map<int, TH2F*>& chists)
{
	...
	Int_t col;
	if (col1>col2) col = col1;
	else col = col2;

	// merge the two into one
	cluster1->Add(cluster2);
	cluster1->SetFillColor(col);
...
Int_t GetJetColor(TH2F* jethist)
{
	...
	if (Nmatch>0){
		...
		std::cout << "GetJetColor: Found a SD-rejected cluster matching to this cluster. Returning color: " << color << std::endl;
		return color;
	}
	...
	if (Nmatch>0){
		...
		std::cout << "GetJetColor: Found a jet without subjets matching to this cluster. Returning color: " << color << std::endl;
		return color;
	}
	...
	if (Nmatch>0){
		color = TColor::GetColor( "#e6ffe6"); // very light green
		std::cout << "GetJetColor: Found a low-pt (sub)jet matching to this cluster. Returning color: " << color << std::endl;
		return color;
	}
...
void PrintStep(int step, std::vector<part> ips, std::vector<leave> tree)
{
  ...
  TString info = TString::Format("Clustering step: %d", ch);
